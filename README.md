...
4. 서버 역할 설정에서 봇의 역할을, 자동 지급/제거할 역할보다 위로 올립니다.
...
버튼별 지급 역할을 설정합니다. `/동아리역할설정`은 기존 버튼 지급 역할을 초기화하고 1개로 설정합니다.

```text
/동아리역할설정 동아리:<선택값> role:@역할
```

같은 버튼에서 지급 역할을 2개 이상 주고 싶으면 `/동아리역할추가`를 더 실행합니다.

```text
/동아리역할추가 동아리:<선택값> role:@추가역할
```

버튼을 눌렀을 때 제거할 역할도 설정할 수 있습니다. `/동아리역할삭제설정`은 기존 버튼 삭제 역할을 초기화하고 1개로 설정합니다.

```text
/동아리역할삭제설정 동아리:<선택값> role:@제거할역할
```

제거할 역할을 2개 이상 두고 싶으면 `/동아리역할삭제추가`를 더 실행합니다.

```text
/동아리역할삭제추가 동아리:<선택값> role:@추가제거역할
```
...
입장 안내 채널에 올라온 본인 소속 버튼을 누르고 이름을 입력합니다. 설정된 버튼 역할이 있으면 서버별명 변경과 함께 해당 역할도 자동으로 지급되고, 삭제 역할이 있으면 해당 역할은 제거됩니다.
...
서버별명은 `동아리명/이름` 형식으로 설정됩니다.

## 자주 막히는 부분

- 자동 역할이 안 들어가면 봇 역할이 지급할 역할보다 위에 있는지 확인하세요.
- 삭제 역할이 안 빠지면 봇 역할이 제거할 역할보다 위에 있는지 확인하세요.
